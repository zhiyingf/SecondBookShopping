...
<c:forEach items="${bookList}" var="book" begin="${(pageNos-1)*3 }" end="${(pageNos*3-1)}">
<div class="booklist0">
  ...
  <div class="booklist03">
     ...
     <div class="orderformBuy">          
       ...
       <a style="text-decoration: none;" href="Prebuy?order_number=${book.order_number}&kind=1">加入购物车</a> |
       <a style="text-decoration: none;" href="Prebuy?order_number=${book.order_number}&kind=2">直接去购买</a>    
     </div>
     </c:if>     
  </div>
</div>
...
<div class="pages">
  ...
  <table class="pagestable" border="1">
    <tr>
      ...
      <td width="55px">
        <c:choose>
           <c:when test="${pageNos>0 }"><a style="text-decoration: none;" href="BookListServlet?pageNos=${pageNos-1}">上一页</a></c:when>
           <c:otherwise><a style="text-decoration: none;" href="BookListServlet?pageNos=1">上一页</a></c:otherwise>
        </c:choose>        
      </td>
      <td width="55px">
           <c:choose>
             <c:when test="${pageNos <countPage }"><a style="text-decoration: none;" href="BookListServlet?pageNos=${pageNos+1 }">下一页</a></c:when>
             <c:otherwise><a style="text-decoration: none;" href="BookListServlet?pageNos=${countPage}">下一页</a></c:otherwise>             
           </c:choose>           
      </td>
      <td width="55px"><a style="text-decoration: none;" href="BookListServlet?pageNos=${countPage}">尾页</a></td>
      <td width="130px"><div>页码${pageNos} / 共${countPage}页</div></td>
    </tr>
  </table>
  </div>
</div>
